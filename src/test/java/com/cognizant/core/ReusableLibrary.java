...
public abstract class ReusableLibrary {
	 ...
	 * 
	 */
	protected void validateLabels(By by, String strValue, String strDescription) {
		try {
			driverUtil.waitUntilElementLocated(by, 30);
			if(driver.findElement(by).getText().equalsIgnoreCase(strValue)) {
				report.updateTestLog("Validate Labels", strDescription + strValue, Status.PASS);
			}else {
				report.updateTestLog("Validate Labels", strDescription + strValue, Status.FAIL);
			}
		}catch(Exception e) {
			report.updateTestLog("Validate Labels", "Error in Validate Labels", Status.FAIL);
		}
	}
